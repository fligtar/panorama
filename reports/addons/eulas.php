...
class AddonEULAs extends Report {
    public $table = 'addons_eulas';
    ...
    public function analyzeDay() {
        $qry = "SELECT status, COUNT(*) FROM (SELECT IF(eula, 'Has EULA', 'No EULA') as status FROM addons WHERE addontype_id < 4) AS temp GROUP BY status ORDER BY status";
    ...
    public function generateCSV($graph) {
        if ($graph == 'current') {
            ...
            $values = $this->db->query_amo("SELECT status, COUNT(*) FROM (SELECT IF(eula, 'Has EULA', 'No EULA') as status FROM addons WHERE addontype_id < 4) AS temp GROUP BY status ORDER BY status");
