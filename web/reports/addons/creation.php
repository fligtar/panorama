...
class AddonCreation extends Report {
    ...
    public function analyzeDay($date = '') {
        ...
        $queries = array(
            ...
            'sdk' => "SELECT COUNT(*) FROM addons INNER JOIN versions ON addons.current_version = versions.id INNER JOIN files ON versions.id = files.version_id AND files.platform_id = 1 WHERE files.jetpack_version IS NOT NULL AND DATE(addons.created) = '%DATE%'",
