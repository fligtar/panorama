...
class CollectionWatchers extends Report {
    public $table = 'collections_watchers';
    ...
    public function analyzeDay($date = '') {
        ...
        $qry = "SELECT collection_type, COUNT(*) FROM collections INNER JOIN collection_subscriptions ON collection_subscriptions.collection_id = collections.id WHERE DATE(collection_subscriptions.created) = '%DATE%' GROUP BY collection_type ORDER BY collection_type";
